...
func (instance *pbftCore) sendViewChange() events.Event {
	...
	instance.innerBroadcast(&Message{Payload: &Message_ViewChange{ViewChange: vc}}, 2)
...
func (instance *pbftCore) recvViewChange(vc *ViewChange) events.Event {
	...
	logger.Infof("Replica %d received view-change from replica %d, v:%d, h:%d, |C|:%d, |P|:%d, |Q|:%d, quorum is %d, wanted %d, result is %v",
	instance.id, vc.ReplicaId, vc.View, vc.H, len(vc.Cset), len(vc.Pset), len(vc.Qset), quorum, instance.allCorrectReplicasQuorum(), quorum >= instance.allCorrectReplicasQuorum())
	
	
	logger.Debugf("Replica %d now has %d view change requests for view %d", instance.id, quorum, instance.view)
	
	
	if quorum >= instance.f+1 && quorum < instance.allCorrectReplicasQuorum(){
		logger.Infof("Replica %d received f+1 view-change messages, triggering view-change",
			instance.id)
		// subtract one, because sendViewChange() increments
		//instance.view = minView - 1
		return instance.sendViewChange()
...
func (instance *pbftCore) sendNewView() events.Event {
	...
	instance.innerBroadcast(&Message{Payload: &Message_NewView{NewView: nv}}, 2)
...
func (instance *pbftCore) processNewView2(nv *NewView) events.Event {
	...
	if instance.primary(instance.view) != instance.id {
		for n, d := range nv.Xset {
			...
			instance.innerBroadcast(&Message{Payload: &Message_Prepare{Prepare: prep}}, 2)
		}
	} else {
