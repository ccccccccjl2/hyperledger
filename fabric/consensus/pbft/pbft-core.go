...
//clerk没有在规定时间内发送feedback
type clerkFeedbackTimeoutEvent struct{}
...
type innerStack interface {
	...
	//增加部分
	broadcastToClerk(msgPayload []byte)
	broadcastToAll(msg *pb.Message)
	unicastToOne(msg *pb.Message, id uint64)
}
...
type pbftCore struct {
	...
	leader uint64
	clerk map[uint64]uint64
	scores map[uint64]int
	FeedbackNum int
	notConsensused int//记录还没有打包的交易
	clientRequests []*Request
	vrfs []uint64
	vrf_peers map[uint64]uint64//map[vrf随机数] 节点id
	ifSendVrf bool
	leaderCheatingNum int
	ifKnowResult bool//是否知道共识结果
	
	
	
	pdigest string//主节点发送的pre-prepare消息中的digest
	pview uint64
	pseq uint64
	prePrepare *PrePrepare
	clerkFeedbackTimer     events.Timer
	clerkFeedbackTimerActive bool
	ifAsking bool
	prepares []*Prepare3
	clerkFeedbackTimeout        time.Duration
	ifSendFeedback bool
	waitTimer     events.Timer
	waitTimerActive bool
...
func newPbftCore(id uint64, config *viper.Viper, consumer innerStack, etf events.TimerFactory) *pbftCore {
	...
	instance.clerkFeedbackTimer = etf.CreateTimer()
	instance.waitTimer = etf.CreateTimer()
	...
	instance.clerk = make(map[uint64]uint64)
	instance.leader = uint64(0)
	instance.scores = make(map[uint64]int)
	instance.FeedbackNum = 0
	instance.notConsensused = 0
	instance.clientRequests = nil
	instance.vrfs = []uint64{}
	instance.vrf_peers = make(map[uint64]uint64)
	instance.ifSendVrf = false
	instance.leaderCheatingNum = 0
	instance.ifKnowResult = false
	instance.clerkFeedbackTimerActive = false
	instance.ifAsking = false
	instance.prepares = []*Prepare3{}
	instance.clerkFeedbackTimeout, err = time.ParseDuration(config.GetString("general.timeout.feedback"))
	instance.clerkFeedbackTimeout = instance.clerkFeedbackTimeout / 2
	instance.ifSendFeedback = false
	instance.waitTimeout, err = time.ParseDuration(config.GetString("general.timeout.wait"))
...
func (instance *pbftCore) ProcessEvent(e events.Event) events.Event {
	...
	case *Prepare:
		...
		err = instance.recvPrepareClerk(et)
	case *Commit:
	...
	case clerkFeedbackTimeoutEvent:
		instance.ifAsking = true
		logger.Infof("replica %v sending ask.", instance.id)
		//节点互相发送prepare消息
		instance.sendPrepare2()
	case *Prepare2:
		err = instance.recvPrepare2(et)
	case *Prepare3:
		err = instance.recvPrepare3(et)
...
func (instance *pbftCore) primary(n uint64) uint64 {
	...
	return instance.leader - uint64(1)
}
...
func (instance *pbftCore) prepared(digest string, v uint64, n uint64) bool {
	...
	cert := instance.certStore[msgID{v, n}]
	if cert == nil {
		//logger.Infof("cert is nil,view = %d", instance.view)
		return false
	}
	...
	//logger.Infof("quorum = %d, intersectionQuorum-1 = %d", quorum, instance.intersectionQuorum() - 1)
...
func (instance *pbftCore) committed(digest string, v uint64, n uint64) bool {
	...
	//logger.Infof("quorum = %v, intersectionQuorum = %v", quorum, instance.intersectionQuorum())
...
func (instance *pbftCore) recvMsg(msg *Message, senderID uint64) (interface{}, error) {
	...
	}else if vrfProve := msg.GetVrfProve(); vrfProve != nil{
		return vrfProve, nil
	}else if feedback := msg.GetFeedback(); feedback != nil{
		return feedback, nil
	}else if leaderHello := msg.GetLeaderHello(); leaderHello != nil{
		return leaderHello, nil
	}else if leaderCheat := msg.GetLeaderCheat(); leaderCheat != nil{
		return leaderCheat, nil
	}else if prep := msg.GetPrepare2(); prep != nil {
		if senderID != prep.ReplicaId {
			return nil, fmt.Errorf("Sender ID included in prepare message (%v) doesn't match ID corresponding to the receiving stream (%v)", prep.ReplicaId, senderID)
		}
		return prep, nil
	}else if prep := msg.GetPrepare3(); prep != nil {
		if senderID != prep.ReplicaId {
			return nil, fmt.Errorf("Sender ID included in prepare message (%v) doesn't match ID corresponding to the receiving stream (%v)", prep.ReplicaId, senderID)
		}
		return prep, nil
...
func (instance *pbftCore) recvRequestBatch(reqBatch *RequestBatch) error {
	...
	//logger.Infof("leader:%d,ifactiveView:%v", instance.primary(instance.view), instance.activeView)
...
func (instance *pbftCore) sendPrePrepare(reqBatch *RequestBatch, digest string) {
	...
	if doByzantine == false{
		...
		instance.pdigest = preprep.BatchDigest
		instance.pview = preprep.View
		instance.pseq = preprep.SequenceNumber
		instance.prePrepare = preprep
		cert.digest = digest
		...
		logger.Infof("leader %d send pre-prepare,view=%d, seq=%d", instance.id, preprep.View, preprep.SequenceNumber)

		instance.innerBroadcast(&Message{Payload: &Message_PrePrepare{PrePrepare: preprep}}, 2)
...
func (instance *pbftCore) recvPrePrepare(preprep *PrePrepare) error {
	...
	if instance.view != preprep.View{
		logger.Infof("pre-prepare from last view,ignore.")
		return nil
	}
	...
	/*if !instance.inWV(preprep.View, preprep.SequenceNumber) {
		if preprep.SequenceNumber != instance.h && !instance.skipInProgress {
			logger.Warningf("Replica %d pre-prepare view different, or sequence number outside watermarks: preprep.View %d, expected.View %d, seqNo %d, low-mark %d", instance.id, preprep.View, instance.view, preprep.SequenceNumber, instance.h)
		} else {
	...
	if cert.prePrepare != nil{
		logger.Infof("have received pre-prepare, ignore.")
		return nil
	}
	...
	//instance.nextNotConsensused += len(preprep.RequestBatch.Batch)
	logger.Infof("replica %d received pre-prepare message", instance.id)
	
	//已经知道投票结果了
	if len(cert.prepare) >= instance.f * 2{
		instance.commitBatch(preprep.View, preprep.SequenceNumber)
		return nil
	}
	...
	if instance.primary(instance.view) != instance.id && instance.prePrepared(preprep.BatchDigest, preprep.View, preprep.SequenceNumber) && !cert.sentPrepare {
		...
		instance.persistQSet()
		for index, _ := range(instance.clerk){//clerk自己给自己发prepare消息
			if index == instance.id{
				instance.recvPrepareClerk(prep)
			}
		}
		//instance.softStartTimer2(instance.requestTimeout, fmt.Sprintf("waiting for clerk's feedback"))
		//instance.innerBroadcast(&Message{Payload: &Message_PrePrepare{PrePrepare: preprep}}, 2)
		return instance.innerBroadcast(&Message{Payload: &Message_Prepare{Prepare: prep}}, 1)
	}
...
func (instance *pbftCore) recvPrepare(prep *Prepare) error {
	...
	commit2 := &Commit{
		...
		ReplicaId:      instance.primary(instance.view),
	}
	...
	for index, _ := range(instance.vrfs){
		instance.vrfs[index] = 0
	}
	...
	//上链
	//logger.Infof("ready to commit")
	//instance.stopTimer()
	instance.lastNewViewTimeout = instance.newViewTimeout
	delete(instance.outstandingReqBatches, string(instance.pdigest))
	instance.executeOutstanding()
	if instance.pseq == instance.viewChangeSeqNo {
		logger.Infof("Replica %d cycling view for seqNo=%d", instance.id, instance.pseq)
		instance.sendViewChange()
	}
	return nil
...
func (instance *pbftCore) maybeSendCommit(digest string, v uint64, n uint64) error {
	...
	if instance.prepared(digest, v, n) && !cert.sentCommit {
		...
		return instance.innerBroadcast(&Message{&Message_Commit{commit}}, 2)
	}
	return nil
}
...
func (instance *pbftCore) executeOutstanding() {
	//logger.Infof("executeOutstanding")
	if instance.currentExec != nil {
...
func (instance *pbftCore) executeOne(idx msgID) bool {
	//logger.Infof("now executing tx, view=%d, id=%d", idx.v, idx.n)
	cert := instance.certStore[idx]

	if idx.n != instance.lastExec+1 || cert == nil {   //|| cert.prePrepare == nil {
		if idx.n != instance.lastExec+1{
			logger.Infof("idx.n=%d, expected n=%d", idx.n, instance.lastExec+1)
		}else if cert == nil{
			logger.Infof("cert is nil, idx.view=%d,idx.seq%d", idx.v, idx.n)
		}else{
			//logger.Infof("cert.prePrepare is nil")
		}
		return false
	}
	...
	/*if !instance.committed(digest, idx.v, idx.n) {
		logger.Infof("replica %d not committed this batch", instance.id)	
		return false
	}*/
...
func (instance *pbftCore) executeOne2(idx msgID) bool {
	//logger.Infof("now executing tx, view=%d, id=%d", idx.v, idx.n)
	cert := instance.certStore[idx]
...
func (instance *pbftCore) Checkpoint(seqNo uint64, id []byte) {
	...
	logger.Warningf("Replica %d preparing checkpoint for view=%d/seqNo=%d and b64 id of %s", instance.id, instance.view, seqNo, idAsString)
	...
	instance.innerBroadcast(&Message{Payload: &Message_Checkpoint{Checkpoint: chkpt}}, 2)
}
...
func (instance *pbftCore) execDoneSync2() {
	if instance.currentExec != nil {
		logger.Infof("Replica %d finished execution %d, trying next", instance.id, *instance.currentExec)
		instance.lastExec = *instance.currentExec
...
func (instance *pbftCore) innerBroadcast(msg *Message, n int) error {
	...
	if doByzantine {
		...
		if n == 1{
			instance.consumer.broadcastToClerk(msgRaw)
			return nil
		}
		if n == 10{
		...
		logger.Warningf("replica %d is a byzantine replica", instance.id)
...
func (instance *pbftCore) softStartTimer2(timeout time.Duration, reason string) {
	logger.Debugf("Replica %d soft starting new view timer for %s: %s", instance.id, timeout, reason)
	instance.newViewTimerReason = reason
	instance.timerActive = true
	instance.newViewTimer.SoftReset(timeout, clerkFeedbackTimeoutEvent{})
}
...
func (instance *pbftCore) startClerkFeedbackTimer() {
	instance.clerkFeedbackTimerActive = true
	instance.clerkFeedbackTimer.Reset(instance.clerkFeedbackTimeout, clerkFeedbackTimeoutEvent{})
}


func (instance *pbftCore) stopClerkFeedbackTimer() {
	instance.clerkFeedbackTimerActive = false
	instance.clerkFeedbackTimer.Stop()
}


func (instance *pbftCore) startWaitTimer() {
	instance.waitTimerActive = true
	instance.waitTimer.Reset(instance.waitTimeout, waitTimeoutEvent{})
}


func (instance *pbftCore) stopWaitTimer() {
	instance.waitTimerActive = false
